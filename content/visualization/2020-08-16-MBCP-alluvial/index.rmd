...
df <- readxl::read_excel("MBCS II_geo_3.xls") %>%
  ...
  select(region_mex, sector_cross, dest_type_us) %>%
  drop_na()
...
df <- df %>%
  ...
  select(region_mex_named, sector_cross, dest_type_us_named)
```

And, plot! 
```{r plot, fig.dim = c(12, 8)}
ggplot(data = df,
  ...
  ggtitle("Where were they migrating?",
          subtitle = "Communities of origin, crossing points, and destination intentions among unauthorized Mexican migrants") +
  labs (caption = "Source: Migrant Border Crossing Study II (Co-PIs Slack, Martínez, and Whiteford)
                    DOI: 10.1080/00380253.2020.1733448
                    Graphic designed by Kelsey E. Gonzalez")
